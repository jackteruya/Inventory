...
class RemoveStockUseCase:
    ...
    async def execute(self, item_id: UUID, quantity: int) -> InventoryItem:
        logger.info("removing stock", extra={"item_id": str(item_id), "quantity": quantity})

        async with self.uow:
            item = await self.repository.get_by_id_for_update(item_id)
            if not item:
                logger.warning("item not found for stock removal", extra={"item_id": str(item_id)})
        ...
        logger.info(
            "stock removed",
            extra={"item_id": str(item.id), "quantity": quantity, "remaining": item.quantity},
        )
